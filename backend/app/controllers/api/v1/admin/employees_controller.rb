  ...
  module V1
    module Admin
      class EmployeesController < ::Api::V1::Admin::ApplicationController
        ...
        def index
          company = Company.find(params[:company_id]&.to_i) if params[:company_id].present?
          respond_with company.nil? ? Employee.all : company.employees, each_serializer: EmployeeSerializer
        end
